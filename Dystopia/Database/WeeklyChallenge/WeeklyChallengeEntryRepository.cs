...
public class WeeklyChallengeEntryRepository : IWeeklyChallengeEntryRepository
{
    ...
    public async Task<WeeklyChallengeEntryEntity?> GetByUserAndChallengeAsync(Guid userId, int weeklyChallengeId)
    {
        return await _dbContext.WeeklyChallengeEntries
            .Include(e => e.User)
            .Include(e => e.WeeklyChallenge)
            .Include(e => e.League)
            .Include(e => e.Game)
            .FirstOrDefaultAsync(e => e.UserId == userId && e.WeeklyChallengeId == weeklyChallengeId);
    }
    ...
    public async Task<List<WeeklyChallengeEntryEntity>> GetLeaderboardAsync(int weeklyChallengeId, int leagueId, int limit = 100)
    {
        return await _dbContext.WeeklyChallengeEntries
            .Include(e => e.User)
            .Include(e => e.Game)
    ...
    {
        var existing = await GetByUserAndChallengeAsync(entry.UserId, entry.WeeklyChallengeId);
    ...
    public async Task<int> GetUserRankAsync(Guid userId, int weeklyChallengeId, int leagueId)
    {
        var userEntry = await _dbContext.WeeklyChallengeEntries
            .FirstOrDefaultAsync(e => e.UserId == userId && e.WeeklyChallengeId == weeklyChallengeId && e.LeagueId == leagueId);

        if (userEntry == null || !userEntry.IsValid)
            return -1;

        var rank = await _dbContext.WeeklyChallengeEntries
            .Where(e => e.WeeklyChallengeId == weeklyChallengeId && e.LeagueId == leagueId && e.IsValid && e.Score > userEntry.Score)
            .CountAsync();
